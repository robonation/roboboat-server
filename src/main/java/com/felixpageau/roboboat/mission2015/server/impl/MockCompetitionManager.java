...
public class MockCompetitionManager implements CompetitionManager {
  ...
  public MockCompetitionManager(Competition competition) throws URISyntaxException, MalformedURLException {
    ...
    URL url = Optional.ofNullable(MockCompetitionManager.class.getResource("/roboboat2015-images/source/")).orElse(
  ...
  public ReportStatus startRun(Course course, TeamCode teamCode) {
    ...
    competition.getActiveRuns().get(course).addEvent(new Event(new DateTime(), String.format("%s - %s - Run started - new setup (%s)", course, teamCode, r)));
  ...
  public ReportStatus endRun(Course course, TeamCode teamCode) {
    ...
    competition.getActiveRuns().get(course).addEvent(new Event(new DateTime(), String.format("%s - %s - Run ended", course, teamCode)));
    return new ReportStatus(true);
  }
  ...
  public GateCode getObstacleCourseCode(Course course, TeamCode teamCode) {
    ...
    competition.getActiveRuns().get(course)
        .addEvent(new Event(new DateTime(), String.format("%s - %s - ObstacleAvoidance - request gatecode (%s)", course, teamCode, code)));
    return code;
  }
  ...
  public DockingSequence getDockingSequence(Course course, TeamCode teamCode) {
    ...
    competition.getActiveRuns().get(course)
        .addEvent(new Event(new DateTime(), String.format("%s - %s - AutomatedDocking - request bay (%s)", course, teamCode, sequence)));
    return sequence;
  }
  ...
  public ReportStatus reportPinger(Course course, TeamCode teamCode, BeaconReport payload) {
    ...
    Event e = new Event(new DateTime(), String.format("%s - %s - ObstacleAvoidance - reported pinger (%s) -> %s", course, teamCode, payload,
  ...
  public ReportStatus reportInterop(Course course, TeamCode teamCode, InteropReport report) {
    ...
    Event e = new Event(new DateTime(), String.format("%s - %s - Interop - reported shape (%s) -> %s", course, teamCode, report, success ? "success"
        : "incorrect"));
    competition.getActiveRuns().get(course).addEvent(e);
    return new ReportStatus(success);
  }
  ...
  public List<String> listInteropImages(Course course, TeamCode teamCode) {
    Event e = new Event(new DateTime(), String.format("%s - %s - Interop - listed image", course, teamCode));
    competition.getActiveRuns().get(course).addEvent(e);
  ...
  public Optional<byte[]> getInteropImage(String filename) {
    Event e = new Event(new DateTime(), String.format("? - ? - Interop - requested image %s", filename));
  ...
  public UploadStatus uploadInteropImage(Course course, TeamCode teamCode, byte[] content) {
    ...
    ra.addEvent(new Event(new DateTime(), String.format("%s - %s - Interop - uploaded image (%s)", course, teamCode, imageId)));
